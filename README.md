...
First, go to the [singleron-RD/nf-celescope releases page](https://github.com/singleron-RD/nf-celescope/releases) and find the latest pipeline version - numeric only (eg. `2.11.3`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 2.11.3`. Of course, you can switch to another version by changing the number after the `-r` flag.
